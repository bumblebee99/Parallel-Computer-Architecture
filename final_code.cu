...
int main(int argc, char *argv[])
{
	...
	double startTime = timerval();
	
												double ohstart0 = timerval();
	...
	cudaMalloc(&d_gMask, n * n * sizeof(float));
							double ohend0 = timerval();

	fp1 = fopen("1024by1024.txt", "r");
	...
	double cannystartTime = timerval();
	double compstart0 = timerval();
	gaussian_filter(h_gMask, sigma);
	double compend0 = timerval();
	
												double ohstart1 = timerval();
	...
	cudaMemset(d_img_op, 0, HEIGHT * WIDTH * sizeof(pixel_t));
												...
												double ohend1 = timerval();
												
												
	const dim3 block(32,32, 1); //888888888888888888888888888888888888888888//
	
	
	const dim3 grid(64, 64); //(32,32)   // (32,32) is normal. //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$//



								double compstart1 = timerval();
	convolution << <grid, block >> >(d_img_ip, d_img_op, d_gMask, n);
	cudaDeviceSynchronize();
								double compend1 = timerval();
								double ohstart2 = timerval();
	...
	cudaMemcpy(d_img_op, h_img_op, HEIGHT * WIDTH * sizeof(pixel_t), cudaMemcpyHostToDevice);
								double ohend2 = timerval();
								double compstart2 = timerval();
	convolution1 << <grid, block >> >(d_img_op, d_gGx, d_mGx, 3);
	cudaDeviceSynchronize();
								double compend2 = timerval();
								double ohstart3 = timerval();
	...
	cudaMemcpy(d_gGx, h_gGx, HEIGHT * WIDTH * sizeof(pixel_t), cudaMemcpyHostToDevice);
								double ohend3 = timerval();
								double compstart3 = timerval();
	convolution1 << <grid, block >> >(d_img_op, d_gGy, d_mGy, 3);
	cudaDeviceSynchronize();
								double compend3 = timerval();
								double ohstart4 = timerval();
	...
	cudaMemset(d_gradient, 0, HEIGHT * WIDTH * sizeof(pixel_t));
								double ohend4 = timerval();
								double compstart4 = timerval();
	pixel_hypot << <grid, block >> >(d_gGx, d_gGy, d_gradient, d_thisAngle);
	cudaDeviceSynchronize();
								double compend4 = timerval();
								double ohstart5 = timerval();
	...
	cudaMemset(d_edgepoints, 0, HEIGHT * WIDTH * sizeof(pixel_t));
								double ohend5 = timerval();
								double compstart5 = timerval();
	findNonMax << <grid, block >> >(d_nonMax, d_thisAngle, d_edgepoints, 15, 20);
	cudaDeviceSynchronize();
								double compend5 = timerval();
								double ohstart6 = timerval();
...
/*
	...
	*/
	...
	cudaDeviceSynchronize();
						double compend8 = timerval();
						double ohstart10 = timerval();
...
*/
	...
	cudaMemcpy(d_dilin, h_edges, sizeof(pixel_t), cudaMemcpyHostToDevice);
					double ohend11 = timerval();
					double compstart9 = timerval();
					double morphstartTime = timerval();
	dilation <<<grid, block >>>(d_edges, d_dilout);
	cudaDeviceSynchronize();
					double compend9 = timerval();
					double ohstart12 = timerval();
	cudaMemcpy(h_dilout, d_dilout, HEIGHT*WIDTH*sizeof(pixel_t), cudaMemcpyDeviceToHost);
					double ohend12 = timerval();
/*
	for (i = 0; i<HEIGHT; i++)
	{
	...
	*/
	...
	memset(h_morout, 0, HEIGHT*WIDTH*sizeof(pixel_t));
					double ohstart13 = timerval();
	cudaMalloc(&d_morout, HEIGHT * WIDTH * sizeof(pixel_t));
	cudaMemset(d_morout, 0, HEIGHT * WIDTH * sizeof(pixel_t));
					double ohend13 = timerval();
					double compstart10 = timerval();
	erosion <<<grid, block >>>(d_dilout, d_morout);
	cudaDeviceSynchronize();
					double compend10 = timerval();
					double ohstart14 = timerval();
	cudaMemcpy(h_morout, d_morout, HEIGHT*WIDTH*sizeof(pixel_t), cudaMemcpyDeviceToHost);
					double ohend14 = timerval();
/*
	for (i = 0; i<HEIGHT; i++)
	{
...
*/	
	double morphendTime = timerval();
	double endTime = timerval();
	
	printf("total time: %.8f\n",endTime-startTime);
	printf("Canny time:%.8f\n", cannyendTime - cannystartTime);
	printf("Hough time:%.8f\n", houghendTime - houghstartTime);
	printf("Morph time:%.8f\n", morphendTime - morphstartTime);
	double  comptime = (compend0 - compstart0) + (compend1 - compstart1) +(compend2 - compstart2) + (compend3 - compstart3) + (compend4 - compstart4) + (compend5 - compstart5) + (compend6 - compstart6) + (compend7 - compstart7) + (compend8 - compstart8) + (compend9 - compstart9) + (compend10 - compstart10) ;
	double ohtime  =   ( ohend0 - ohstart0) + ( ohend1 - ohstart1) + ( ohend2 - ohstart2) + ( ohend3 - ohstart3) + ( ohend4 - ohstart4) + ( ohend5 - ohstart5) + ( ohend6 - ohstart6)+ ( ohend7 - ohstart7) + ( ohend8 - ohstart8) + ( ohend9 - ohstart9) + ( ohend10 - ohstart10) + ( ohend11 - ohstart11) + ( ohend12 - ohstart12) + ( ohend13 - ohstart13) + ( ohend14 - ohstart14);
	printf("Computation time: %.8f\n", comptime);
	printf("Commmunication time: %.8f\n", ohtime);
	printf("Computation to Commmunication time: %.8f\n", comptime/ohtime);
	return 0;
}